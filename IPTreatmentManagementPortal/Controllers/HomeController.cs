    ...
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> PostLogin([FromForm] User cred)
        {
            if (ModelState.IsValid)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PostAsync("https://authorization-microservice.azurewebsites.net/api/Auth", content))
                    {
        ...
        public async Task<ActionResult> GetPackages()
        {
            using (var client = new HttpClient())
            {
                if (token == null)
                {
                    ...
                    string url = "https://treatment-offerings.azurewebsites.net/api/Offering/IPTreatmentPackages";
        ...
        public async Task<ActionResult> NewPatient([FromForm] Patient patient)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.GetAsync("https://treatment-microservice.azurewebsites.net/api/Treatment/FormulateTreatmentTimetable?Id=" + pId + "&Name=" + patient.Name + "&Age=" + patient.Age + "&Ailment=" + patient.Ailment + "&TreatmentPackageName=" + patient.TreatmentPackageName + "&CommencementDate=" + patient.CommencementDate))
                    {
        ...
        public async Task<ActionResult> GetInsurerDetail()
        {
            ...
            if (token == null)
            {
                ...
                string url = "https://insurance-claim.azurewebsites.net/api/Insurance/AllInsurerDetail";
                using (var response = await client.GetAsync(url))
                {
                    var apiResponse = await response.Content.ReadAsStringAsync();
            ...
            if (ModelState.IsValid)
                {
                    ...
                    using (var httpClient = new HttpClient())
                    {
                        ...
                        using (var response = await httpClient.PostAsync("https://insurance-claim.azurewebsites.net/api/Insurance/GetBalance", content))
                        {
